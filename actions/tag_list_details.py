...
class TagListDetails(BaseAction):
    def run(self, **kwargs):
        """
        List all tags from a given category or all tags with full details

        Args:
        - kwargs: inputs to the action
          - category_name: Category to list tags from (optional)
          - vsphere: Pre-configured vsphere connection details (config.yaml)

        Returns:
        - list: List of all tag details including id, name, description, category_id
        """
        ...
        tag_ids = self.tagging.tag_list(category_id)
        tags = []

        for tag_id in tag_ids:
            tag_details = self.tagging.tag_get(tag_id)
